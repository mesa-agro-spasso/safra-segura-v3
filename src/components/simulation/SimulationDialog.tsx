...
import { Calculator, FileDown, Save, Trash2, RefreshCw, FolderOpen, Info } from 'lucide-react';
...
export function SimulationDialog({ open, onOpenChange, currentBatchCreatedAt }: SimulationDialogProps) {
  ...
  const { data: drafts } = useSimulationDrafts(open);
  ...
  const handleCalculate = async () => {
    ...
    if (trioError) { toast.error(trioError); return; }
    const error = validateSimulationForm(form, quote);
            ...
            <TabsContent value="sim" className="mt-3">
              <div className="grid gap-6 lg:grid-cols-2">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <F label="Commodity">
                      <Select
                        ...
                        onValueChange={(v) => patch({ commodity: v as SimulationForm['commodity'] })}
                      >
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                    ...
                    <F label="Bolsa">
                      <Select
                        ...
                        onValueChange={(v) => patch({ benchmark: v as SimulationForm['benchmark'] })}
                      >
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                    ...
                    <F label="Ticker">
                      <Input
                        ...
                        placeholder="ex: ZSX26"
                      />
                    </F>
                    <F label="Preço de futuros">
                      <NumericInput
                        precision={4}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <F label="Data de negócio">
                      <Input type="date" value={form.trade_date} onChange={(e) => patch({ trade_date: e.target.value })} />
                    </F>
                    <F label="Vencimento do contrato">
                      <Input type="date" value={form.exp_date ?? ''} onChange={(e) => patch({ exp_date: e.target.value || null })} />
                    </F>
                    <F label="Venda">
                      <Input type="date" value={form.sale_date ?? ''} onChange={(e) => patch({ sale_date: e.target.value || null })} />
                    </F>
                    <F label="Recepção do grão">
                      <Input
                        type="date"
                        value={form.grain_reception_date ?? ''}
                        onChange={(e) => patch({ grain_reception_date: e.target.value || null })}
                      />
                    </F>
                    <F label="Pagamento">
                      <Input
                  ...
                  <Separator />
                  <p className="text-xs font-medium text-muted-foreground">Preço e ajustes</p>
                  <div className="grid grid-cols-2 gap-3">
                    {form.pricing_method === 'LONG_BASIS' ? (
                      <F label="Basis alvo (R$/sc)">
                        <NumericInput precision={2} value={form.target_basis} onChange={(v) => patch({ target_basis: v })} />
                      </F>
                    ) : (
                      <F label="Preço líquido alvo (R$/sc)">
                        <NumericInput
                          precision={2}
                    ...
                    <F label="Dólar à vista">
                      <NumericInput precision={4} value={form.spot_usd_brl} onChange={(v) => patch({ spot_usd_brl: v })} />
                    </F>
                    <F label="Câmbio override">
                      <NumericInput
                        precision={4}
                        value={form.exchange_rate_override}
                        onChange={(v) => patch({ exchange_rate_override: v })}
                      />
                    </F>
                    <F label="Incremento de arredondamento">
                      <NumericInput
                        precision={2}
                        value={form.rounding_increment}
                        onChange={(v) => patch({ rounding_increment: v })}
                      />
                    </F>
                  </div>

                  <Separator />
                  <p className="text-xs font-medium text-muted-foreground">Custos (manual override)</p>
                  <div className="grid grid-cols-2 gap-3">
                  ...
                  <Button className="w-full" onClick={handleCalculate} disabled={calculating || !numericValid}>
                ...
                {/* ---------- Resultado ---------- */}
                <div className="space-y-4">
                  <h4 className="text-sm font-semibold">Resultado (DRE)</h4>
                  {result ? (
      ...
      <AlertDialog open={confirmClose} onOpenChange={setConfirmClose}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => { setConfirmClose(false); onOpenChange(false); resetAll(); }}
            >
              OK
            </AlertDialogAction>
          </AlertDialogFooter>
